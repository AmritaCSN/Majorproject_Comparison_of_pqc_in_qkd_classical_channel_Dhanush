...
def encrypt(pub_key_file, input_arr, bin_output=False, block=False):
    ...
    ntru.h_poly = Poly(pub_key['h'].astype(np.int64)[::-1], x).set_domain(ZZ)
    if not block:
...
def decrypt(priv_key_file, input_arr, bin_input=False, block=False):
    ...
    ntru.f_poly = Poly(priv_key['f'].astype(np.int64)[::-1], x).set_domain(ZZ)
    ntru.f_p_poly = Poly(priv_key['f_p'].astype(np.int64)[::-1], x).set_domain(ZZ)
...
if __name__ == '__main__':
    ...
    if args['--debug']:
        ...
        ch = input("Chose option \n sign \n verify \n :")
        k = int(input("enter version :"))
        if ch == "verify":
   
            Msg = input("input msg to be verified")
            msg = bytes(Msg,"utf8")
            ...
            sk = falcon.SecretKey(k)
            pk = falcon.PublicKey(sk)
            Msg = input("input msg to be signed")
            msg = bytes(Msg,"utf8")
            sk.sign(msg)
            sig = sk.sign(msg)

            save_object(pk,"pk2")
            save_object(sig,"sig2")
        sys.exit()
    ...
    if not args['gen']:
        if poly_output:
            ...
            sys.stdout.buffer.write(np.packbits(np.array(output).astype(np.int64)).tobytes())
