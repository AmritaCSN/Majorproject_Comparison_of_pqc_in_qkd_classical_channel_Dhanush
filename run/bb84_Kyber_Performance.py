...
def alice(host, receiver, alice_basis, alice_bits, alice_encoded):
    ...
    if enc_basis_from_bob:
        ...
        key_extracted = pub_key["sk"].tobytes()
        # Decypting Bob Basis
        dec_start = time.time()
        basis_from_bob = Kyber512.dec(enc_content_bob, key_extracted, 48)
        dec_end = time.time()
...
def bob(host, receiver, bob_basis):
    ...
    if enc_basis_from_alice:
        ...
        key_extracted_bob = pub_key_bob["sk"].tobytes()
        basis_from_alice = Kyber512.dec(enc_content_alice, key_extracted_bob, 48)
